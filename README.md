# Linked List Project in C

🚀 This project implements various types of linked lists and their associated operations in **C**.

## 🛠 Implemented Features
- **Singly Linked List (SLL)**
- **Doubly Linked List (DLL)**
- **Circular Singly Linked List (CSLL)**
- **Circular Doubly Linked List (CDLL)**
- **Queue Implementation using Singly Linked List**
- **Stack Implementation using Singly Linked List**
- **Sum of Two Polynomials using Singly Linked Lists**
- **Concatenation of Two Singly Linked Lists**

## 📁 Project Structure
The project is organized into different header files for each type of linked list, and all functionalities are implemented in `linked_list.c`.

### Files:
- `Sll.h` – Header file for **Singly Linked List**
- `Dll.h` – Header file for **Doubly Linked List**
- `Cll.h` – Header file for **Circular Singly Linked List**
- `linked_list.c` – Main file that implements the logic for all the linked list types and operations

## 📦 How to Run the Code

1️⃣ **Clone the Repository**
```sh
git clone https://github.com/Rakesh-R-K/Linked_Lists.git
cd linked-list-project
```

2️⃣ **Ensure you have a C compiler installed** (like **GCC**)

3️⃣ **Compile the code**
```sh
gcc linked_list.c Sll.h Dll.h Cll.h -o linked_list.exe
```

4️⃣ **Run the executable**
```sh
./linked_list.exe
```

## 🔧 Requirements
- **GCC Compiler**
- **Any system with C standard library support**

## 🚀 Future Enhancements
- Implementing additional linked list operations

---
💡 **Made with C for efficient data structure handling!** 🚀
